...
import { getStudents } from "../service/api/librarian/GetApi";
...
const LibrarianDashboard = () => {
  ...
  const fetchData = async (tab) => {
    ...
    try {
      ...
      setStudentDetails(await response.data);
    } catch (error) {
            ...
            <ul className="list-disc pl-5">
              {studentDetails.map((student) => (
                <li key={student._id} className="mb-2">
                  <Link to={`/librarian/student/${student._id}`} className="font-medium">
  ...
  return (
    <div className="staff-dashboard p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-center mb-6">Librarian Dashboard</h1>
